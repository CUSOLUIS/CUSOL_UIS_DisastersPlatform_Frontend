...
import { submitFoodOffer } from "./reportSubmission";
import {
  DEFAULT_NOTIFICATION_RADIUS_KM,
  FOOD_OFFER_MIN_DISTINCT_WORDS,
  MAX_ADDRESS_LENGTH,
  ...
  type FoodOfferDraft,
  type FoodOfferReceipt,
} from "./types";

// CHG-163 — «Ofrecer comida»: formulario público (anónimos y
// registrados) con la plantilla «Necesitamos ayuda» (CHG-125/130) y
// el acento de la categoría `community_meal` del mapa.

// El verde lima de la categoría `community_meal` (categoryMeta).
const MEAL_ACCENT = "#a3e635";

export const initialFoodOfferDraft: FoodOfferDraft = {
  description: "",
  address: "",
  latitude: "",
  longitude: "",
  durationValue: "",
  durationUnit: "hours",
  // Radio de aviso con un valor inicial razonable; editable y borrable
  // (vacío = sin aviso de proximidad).
  notificationRadiusKm: String(DEFAULT_NOTIFICATION_RADIUS_KM),
  truthConfirmed: false,
};

export type FoodOfferIssueField = keyof FoodOfferDraft | "location";

export interface FoodOfferIssue {
  field: FoodOfferIssueField;
  message: string;
}

// Espejo del contrato (`FoodOfferInput`): las mismas reglas que
// aplicará el backend, avisadas antes del viaje.
export function collectFoodOfferIssues(draft: FoodOfferDraft): FoodOfferIssue[] {
  const issues: FoodOfferIssue[] = [];
  const push = (field: FoodOfferIssueField, message: string) =>
  ...
  if (description.length < MIN_DESCRIPTION_LENGTH) {
    push(
      "description",
      `Describe la comida que ofreces con al menos ${MIN_DESCRIPTION_LENGTH} caracteres.`,
    );
    ...
    // Mismas reglas de calidad de texto que el backend (CHG-107),
    // avisadas antes de enviar con un mensaje claro en vez del 422
    // opaco.
  ...
  // El punto en el mapa es opcional — la dirección escrita basta. Solo
  // se valida el rango cuando sí hay coordenadas.
  ...
  // La vigencia se valida según la unidad elegida; al enviar los días
  // se convierten a horas (mismo tope del backend: 720).
  ...
  // Si se indica radio de aviso, debe ser un entero 1-100. Vacío es
  // válido (sin aviso); sin coordenadas el radio no viaja.
  ...
  if (!draft.truthConfirmed) {
    push("truthConfirmed", "Debes confirmar que la oferta es real.");
  }
...
export function FoodOfferForm({
  ...
  sessionSource,
  locateVisitor,
  submitOffer = submitFoodOffer,
}: FoodOfferFormProps) {
  const { width } = useWindowDimensions();
  const compact = width < 760;
  const session = useSessionAccount(sessionSource);
  const [draft, setDraft] = useState<FoodOfferDraft>(initialFoodOfferDraft);
  const [formErrors, setFormErrors] = useState<string[]>([]);
  const [submitting, setSubmitting] = useState(false);
  const [receipt, setReceipt] = useState<FoodOfferReceipt | null>(null);
  ...
  const setField = <Key extends keyof FoodOfferDraft>(
    key: Key,
    value: FoodOfferDraft[Key],
  ...
  const submit = async () => {
    const issues = collectFoodOfferIssues(draft);
    ...
    try {
      ...
      setFormErrors([
        error instanceof Error
          ? error.message
          : "No fue posible enviar la oferta. Intenta nuevamente.",
      ]);
  ...
  if (receipt) {
    return <OfferConfirmation receipt={receipt} onHome={onHome ?? onBack} />;
  }

  return (
    <MapScrollLockProvider value={scrollLock}>
    <LinearGradient
      colors={["#0a0d07", colors.canvas, "#090b07"]}
      style={styles.root}
    >
      <SafeAreaView edges={["top"]} style={styles.safeArea}>
        ...
        <InnerRouteHeader
          ...
          session={session}
        />
        <View style={styles.header} testID="food-offer-action-bar">
          <Pressable
          ...
          <View style={[styles.content, compact && styles.contentCompact]}>
            <View style={styles.intro}>
              ...
              <Text style={styles.introText}>
                Publica qué comida ofreces y dónde. La oferta aparece de
                inmediato en el mapa para cualquier persona, con o sin
                cuenta, y deja de mostrarse sola cuando vence la vigencia
                que definas.
              </Text>
            </View>

            <ReportConsiderations
              purpose={reportActionCatalog["community-meals"].purpose}
              considerations={[
                "Este módulo sirve para compartir alimentos preparados con la comunidad durante la emergencia.",
                "La descripción, la dirección y el punto del mapa son públicos: así la comunidad sabe dónde hay comida disponible.",
                "Describe qué ofreces, para cuántas personas alcanza y en qué horario; no incluyas datos personales sensibles.",
                "La plataforma no verifica la preparación de los alimentos: la oferta es un acto voluntario entre vecinos.",
                "Define una vigencia realista en horas o días: al vencer, la oferta desaparece automáticamente de todos los espacios.",
              ]}
            ...
            <FormSection
              code="01"
              title="Qué ofreces"
              description="Describe la comida, para cuántas personas y en qué horario."
            ...
            <FormSection
              code="02"
              title="Dónde"
              description="Escribe la dirección o elígela en el mapa; el punto es necesario para dibujar el marcador."
              onPosition={registerSection}
            >
              <FormField
                label="Dirección *"
              ...
              {/* Patrón CHG-131: a cuántos km a la redonda se avisa en
                  la app instalada. Requiere el punto en el mapa; sin
                  punto el radio no viaja. */}
              <FieldGrid>
                <FormField
            ...
            <FormSection
              code="03"
              title="Vigencia"
              description="Cuánto tiempo debe permanecer visible la oferta, en horas o días."
              onPosition={registerSection}
            >
              <FieldGrid>
                <FormField
              ...
              {/* CHG-130 (patrón): unidad de la vigencia. */}
              <View
              ...
              <Text style={styles.fieldNote}>
                Al vencer, la oferta deja de mostrarse automáticamente en el
                mapa, la portada y los espacios personales. El control lo
            ...
            >
              <Pressable
                ...
                accessibilityLabel="Confirmo que la oferta es real y de buena fe."
              ...
              >
                ...
                <Text style={styles.consentText}>
                  Confirmo que la oferta es real y de buena fe.
                </Text>
              </Pressable>
            </FormSection>
            ...
            {formErrors.length > 0 && (
              <View style={styles.errorSummary} accessibilityRole="alert">
                <Text style={styles.errorSummaryTitle}>
                  Revisa la oferta antes de continuar
                </Text>
            ...
            >
              <View style={styles.submitCopy}>
                ...
                <Text style={styles.submitText}>
                  La oferta se publica de inmediato con un marcador en el
                  mapa y desaparece sola al vencer su vigencia. Recibirás
                  una constancia con el código de la oferta.
                </Text>
              </View>
              <Pressable
                accessibilityRole="button"
                accessibilityLabel="Publicar oferta de comida"
                disabled={submitting}
                onPress={() => void submit()}
                style={({ pressed }) => [
                  styles.submitButton,
              ...
              >
                {submitting ? (
                  <ActivityIndicator color="#0a1207" />
                ) : (
...
function OfferConfirmation({
  ...
  receipt: FoodOfferReceipt;
  onHome: () => void;
}) {
  return (
    <LinearGradient
      colors={["#0b1207", colors.canvas]}
    ...
    >
      <View style={styles.confirmationCard}>
        ...
        <Text style={styles.confirmationText}>
          Tu oferta ya aparece en el mapa y en la portada. Dejará de
          mostrarse automáticamente el{" "}
          {expiryFormatter.format(new Date(receipt.expiresAt))}.
        </Text>
        <View style={styles.receiptCode}>
          <Text style={styles.receiptLabel}>CÓDIGO DE LA OFERTA</Text>
